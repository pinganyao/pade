...
def compute_errors(original_func, approx_func, xs):
    ...
    if not errors:
        return errors, 0.0
    mean_square = sum(err * err for err in errors) / len(errors)
    rms_error = math.sqrt(mean_square)
    return errors, rms_error


def write_csv(filename, xs, errors):
    with open(filename, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["index", "x-value", "pointwise error (4 s.f.)"])
        for idx, (x_val, err) in enumerate(zip(xs, errors), start=1):
            err_str = format_sigfigs(err, 4)
            writer.writerow([idx, format_float(x_val), err_str])
...
def main():
    ...
    bound = 0.8 * radius_val
    xs = build_grid(-bound, bound, step)

    taylor_errors, taylor_rms = compute_errors(original_func, taylor_func, xs)
    pade_errors, pade_rms = compute_errors(original_func, pade_func, xs)

    taylor_csv = unique_filename("taylor-inside-radius.csv")
    pade_csv = unique_filename("pade-inside-radius.csv")

    write_csv(taylor_csv, xs, taylor_errors)
    write_csv(pade_csv, xs, pade_errors)

    print("Finished.")
    print(f"Taylor RMS: {format_sigfigs(taylor_rms)}")
    print(f"Pade RMS:   {format_sigfigs(pade_rms)}")
